...
import { AlertTriangle, Calendar, Building2, Clock, ExternalLink, Zap, Home, Eye, Mail, Loader2, CheckCheck } from 'lucide-react'
...
const PRIORITY = {
  ...
  high: {
    ...
    days: 'bg-orange-50 text-orange-700 border-orange-200',
    label: 'عاجل',
  },
  medium: {
...
function getTypeIcon(type: Alert['type']) {
  switch (type) {
    case 'commercial_registration_expiry': return <Building2 className="h-4 w-4" />
    case 'power_subscription_expiry': return <Zap className="h-4 w-4" />
    case 'moqeem_subscription_expiry': return <Home className="h-4 w-4" />
    ...
    <div className={`group relative flex flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-200/60 ${isRead ? 'opacity-55' : ''}`}>
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div className="flex items-center gap-1.5 rounded-lg bg-slate-50 px-3 py-2">
          ...
          <div className="min-w-0">
            <p className="truncate text-[12px] font-semibold text-slate-800">{alert.company.name}</p>
      ...
      <div className="flex items-center gap-2 border-t border-slate-100 px-4 py-2.5">
        ...
        >
          {actionLoading === 'view' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ExternalLink className="h-3.5 w-3.5" />}
          عرض المؤسسة
        </button>
