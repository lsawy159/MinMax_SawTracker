...
import { BarChart3, RefreshCw, Download, AlertTriangle, Calendar, Wallet, Plus, Loader2, ReceiptText, Eye, FileUp, CheckCircle, Trash2, X, Search } from 'lucide-react'
...
import { useCreatePayrollRun, useDeletePayrollRun, usePayrollRunEntries, usePayrollRunSlips, usePayrollRuns, useScopedPayrollEmployees, useUpsertPayrollEntry, useUpdatePayrollRunStatus } from '@/hooks/usePayroll'
...
const PAYROLL_EXCEL_HEADERS = {
  ...
  transfer_renewal_amount: ['قسط رسوم نقل وتجديد', 'رسوم نقل وتجديد', 'transfer_renewal_amount', 'transfer renewal amount'],
  ...
  installment_deducted_amount: ['خصم الأقساط', 'خصم الاقساط', 'installment_deducted_amount', 'installment deducted amount'],
  overtime_notes: ['ملاحظات الإضافي', 'ملاحظات الاضافي', 'overtime_notes', 'overtime notes'],
  deductions_notes: ['ملاحظات الخصومات', 'ملاحظات الحسومات', 'deductions_notes', 'deductions notes'],
  notes: ['ملاحظات', 'notes'],
} as const

const REQUIRED_PAYROLL_EXCEL_FIELDS: Array<keyof typeof PAYROLL_EXCEL_HEADERS> = ['residence_number']
...
export default function PayrollDeductions() {
  ...
  const [payrollImportPreviewRows, setPayrollImportPreviewRows] = useState<PayrollExcelPreviewRow[]>([])
  ...
  const { data: payrollRuns = [], isLoading: payrollRunsLoading, refetch: refetchPayrollRuns } = usePayrollRuns()
  const { data: payrollEntries = [], isLoading: payrollEntriesLoading, refetch: refetchPayrollEntries } = usePayrollRunEntries(selectedPayrollRunId ?? undefined)
  const { data: payrollSlips = [], refetch: refetchPayrollSlips } = usePayrollRunSlips(selectedPayrollRunId ?? undefined)
  ...
  const { data: scopedPayrollEmployees = [], isLoading: scopedEmployeesLoading } = useScopedPayrollEmployees(
  ...
  const selectedPayrollEmployee = scopedPayrollEmployees.find((employee) => employee.id === payrollEntryForm.employee_id) ?? null
  const payrollSlipEntryIds = useMemo(
    () => new Set(payrollSlips.map((slip) => slip.payroll_entry_id)),
    [payrollSlips]
  )
  const selectedPayrollSlip = payrollSlips.find((slip) => slip.payroll_entry_id === selectedPayrollSlipEntryId) ?? null
  const baseSalary = Number(payrollEntryForm.basic_salary_snapshot || selectedPayrollEmployee?.salary || 0)
  ...
  const selectedPayrollRunEditable = Boolean(
    selectedPayrollRun && selectedPayrollRun.status !== 'finalized' && selectedPayrollRun.status !== 'cancelled'
  )
  ...
  const compactButtonBaseClass = 'h-9 px-3 text-sm font-medium rounded-lg transition inline-flex items-center justify-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed'
  const outlineCompactButtonClass = `${compactButtonBaseClass} bg-white border border-slate-300 text-slate-700 hover:bg-slate-50`
  ...
  useEffect(() => {
    setSelectedPayrollExportRunIds((current) => {
      const next = current.filter((runId) => payrollRunList.some((run) => run.id === runId && run.entry_count > 0))
      const isUnchanged = next.length === current.length && next.every((runId, index) => runId === current[index])
  ...
  useEffect(() => {
    if (selectedPayrollRun && scopedPayrollEmployees.length > 0 && !scopedPayrollEmployees.some((employee) => employee.id === payrollEntryForm.employee_id)) {
  ...
  useEffect(() => {
    if (selectedPayrollEmployee) {
      const existingEntry = payrollEntries.find((entry) => entry.employee_id === selectedPayrollEmployee.id)
      ...
      if (existingEntry) {
        ...
        setPayrollEntryForm((current) => {
          const next = {
            ...
            basic_salary_snapshot: Number(existingEntry.basic_salary_snapshot) || Number(selectedPayrollEmployee.salary || 0),
      ...
      setPayrollEntryForm((current) => {
        const next = {
          ...current,
          basic_salary_snapshot: Number(current.basic_salary_snapshot || selectedPayrollEmployee.salary || 0),
          advance_amount: current.advance_amount === 0
            ? selectedPayrollEmployee.suggested_installment_amount
            : current.advance_amount,
          installment_deducted_amount: current.installment_deducted_amount === 0
            ? selectedPayrollEmployee.suggested_installment_amount
  ...
  const loadPayrollInsights = async () => {
    try {
      ...
      const [{ data: entriesData, error: entriesError }, { data: componentsData, error: componentsError }, { data: employeesData, error: employeesError }, { data: obligationLinesData, error: obligationLinesError }] = await Promise.all([
        supabase
          .from('payroll_entries')
          .select('*, payroll_run:payroll_runs(payroll_month, status)')
      ...
      const employeeMetaMap = new Map<string, { name: string; residence_number: string; project_name: string }>()
      ;(employeesData || []).forEach((employee) => {
        const projectRelation = Array.isArray(employee.project) ? employee.project[0] : employee.project
      ...
      ;(componentsData || []).forEach((component) => {
        ...
        const current = normalizePayrollObligationBreakdown(breakdownByEntryId.get(component.payroll_entry_id as string))
        current[bucket] += Number(component.amount || 0)
        breakdownByEntryId.set(component.payroll_entry_id as string, current)
      })

      const nextObligationRows = ((obligationLinesData || []) as Array<Record<string, unknown>>).map((line) => {
      ...
      const obligationSummaryByEmployeeMonth = new Map<string, { total: number; paid: number; remaining: number }>()
      nextObligationRows.forEach((line) => {
        const monthKey = String(line.due_month || '').slice(0, 7)
        const key = `${line.employee_id}::${monthKey}`
        const current = obligationSummaryByEmployeeMonth.get(key) ?? { total: 0, paid: 0, remaining: 0 }
        current.total += Number(line.amount_due || 0)
        current.paid += Number(line.amount_paid || 0)
        current.remaining += Math.max(Number(line.amount_due || 0) - Number(line.amount_paid || 0), 0)
        obligationSummaryByEmployeeMonth.set(key, current)
      })

      const nextSearchRows = ((entriesData || []) as Array<Record<string, unknown>>).map((entry) => {
        const meta = employeeMetaMap.get(String(entry.employee_id))
        const payrollRunMeta = (entry.payroll_run as { payroll_month?: string; status?: string } | null) ?? null
        const payrollMonthLabel = String(payrollRunMeta?.payroll_month || '').slice(0, 7)
        const obligationSummary = obligationSummaryByEmployeeMonth.get(`${String(entry.employee_id)}::${payrollMonthLabel}`)
        ...
        return {
          ...
          residence_label: String(entry.residence_number_snapshot || meta?.residence_number || ''),
  ...
  const filteredPayrollSearchRows = useMemo(() => {
    ...
    return allPayrollSearchRows.filter((row) => {
      const matchesQuery = !normalizedQuery
        ...
        || String(row.residence_label || '').toLowerCase().includes(normalizedQuery)
        || String(row.project_label || '').toLowerCase().includes(normalizedQuery)
  ...
  const projectFilterOptions = useMemo(() => {
    const values = Array.from(new Set(allPayrollSearchRows.map((row) => row.project_label).filter(Boolean)))
  ...
  const filteredPayrollRunList = useMemo(() => {
    return payrollRunList.filter((run) => {
      const matchesMonth = !payrollRunStatsMonth || run.payroll_month.slice(0, 7) === payrollRunStatsMonth
  ...
  const payrollRunCardsStats = useMemo(() => {
    ...
    return payrollRunStatsRows.reduce(
      (totals, row) => {
        ...
        totals.transferRenewal = roundPayrollAmount(totals.transferRenewal + Number(row.deduction_breakdown.transfer_renewal || 0))
        totals.penalty = roundPayrollAmount(totals.penalty + Number(row.deduction_breakdown.penalty || 0))
        totals.advance = roundPayrollAmount(totals.advance + Number(row.deduction_breakdown.advance || 0))
        totals.other = roundPayrollAmount(totals.other + Number(row.deduction_breakdown.other || 0))
        totals.totalObligations = roundPayrollAmount(totals.totalObligations + Number(row.total_deductions || 0))
        return totals
      },
      {
  ...
  const buildPayrollExportRows = (
  ...
  ): PayrollExportRow[] => entries.map((entry: PayrollEntry) => {
    const breakdown = normalizePayrollObligationBreakdown(
    ...
    return {
      ...
      'المؤسسة': entry.company_name_snapshot || (run.scope_type === 'company' ? getPayrollScopeName(run.scope_type, run.scope_id) : '-'),
      'المشروع': entry.project_name_snapshot || (run.scope_type === 'project' ? getPayrollScopeName(run.scope_type, run.scope_id) : '-'),
  ...
  const sanitizePayrollFileName = (value: string) => value
    .replace(/[\\/:*?"<>|]/g, '-')
    .replace(/\s+/g, '_')
  ...
  const allExportablePayrollRunsSelected = exportablePayrollRunIds.length > 0
  ...
  const fetchPayrollEntryBreakdowns = async (entryIds: string[]) => {
    ...
    ;(data || []).forEach((component) => {
      ...
      const current = normalizePayrollObligationBreakdown(map.get(component.payroll_entry_id as string))
      current[bucket] += Number(component.amount || 0)
  ...
  const buildPayrollExportWorkbook = (XLSX: Awaited<ReturnType<typeof loadXlsx>>, run: NonNullable<typeof selectedPayrollRun>, entries: PayrollEntry[], breakdownByEntryId: Map<string, PayrollObligationBreakdown>) => {
    ...
    const totalGross = rows.reduce((sum, row) => roundPayrollAmount(sum + Number(row['إجمالي الراتب'] || 0)), 0)
    const totalNet = rows.reduce((sum, row) => roundPayrollAmount(sum + Number(row['صافي الراتب'] || 0)), 0)
    const headers = ['اسم الموظف', 'رقم الإقامة', 'المؤسسة', 'المشروع', 'إجمالي الراتب', 'صافي الراتب', 'قسط رسوم نقل وتجديد', 'قسط جزاءات وغرامات', 'قسط سلفة', 'قسط أخرى', 'إجمالي الاستقطاعات', 'أيام الحضور', 'الإجازات المدفوعة', 'الحالة', 'ملاحظات']
    const dataRows = rows.map((row) => headers.map((header) => row[header as keyof PayrollExportRow]))
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
      ...
      ['طريقة الإدخال', getPayrollInputModeText(run.input_mode), 'عدد الموظفين', String(rows.length), 'صافي المسير', totalNet.toLocaleString('en-US')],
  ...
  const downloadPayrollRunExcel = async (run: NonNullable<typeof selectedPayrollRun>, entries: PayrollEntry[], breakdownByEntryId: Map<string, PayrollObligationBreakdown>) => {
  ...
  const handleCreatePayrollRun = async () => {
    ...
    if (!payrollForm.scope_id) {
      toast.error(payrollForm.scope_type === 'project' ? 'يرجى اختيار المشروع' : 'يرجى اختيار المؤسسة')
      return
    }

    const requestedPayrollMonth = `${payrollForm.payroll_month}-01`
    const existingRun = payrollRunList.find((run) => (
    ...
    try {
      ...
      if (isDuplicateRunError) {
        const matchingRun = payrollRunList.find((run) => (
          run.payroll_month.slice(0, 7) === payrollForm.payroll_month
          && run.scope_type === payrollForm.scope_type
          && run.scope_id === payrollForm.scope_id
        ))
  ...
  const handleUpsertPayrollEntry = async () => {
    ...
    try {
      await upsertPayrollEntry.mutateAsync({
        ...
        installment_deducted_amount: deductionBreakdown.transfer_renewal + deductionBreakdown.advance,
  ...
  const handlePrintPayrollSlip = () => {
    ...
    const componentRows = selectedSlipComponents.length > 0
      ? selectedSlipComponents.map((component) => `
          <tr>
  ...
  const handleConfirmPayrollExcelImport = async () => {
    ...
    try {
      ...
      await loadPayrollInsights()
      toast.success(`تم اعتماد ${payrollImportPreviewRows.length} مدخل راتب من ملف Excel وربطه بالالتزامات`)
      setPayrollImportPreviewRows([])
      setPayrollImportFileName('')
  ...
  const handlePayrollExcelImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const sheetHeaders = Object.keys(rows[0] ?? {}).map((header) => normalizePayrollExcelHeader(header))
      ...
      if (missingRequiredHeaders.length > 0) {
        const missingHeadersText = missingRequiredHeaders.map((fieldKey) => PAYROLL_EXCEL_HEADERS[fieldKey][0]).join('، ')
        setPayrollImportHeaderError(`الملف لا يحتوي على الأعمدة المطلوبة التالية: ${missingHeadersText}`)
        toast.error('هيكل ملف الرواتب غير صحيح')
        return
      }

      const scopedEmployeesByResidence = new Map(
        scopedPayrollEmployees.map((employee) => [normalizeResidenceNumber(employee.residence_number), employee])
      )
      ...
      const normalizedRows: PayrollExcelRow[] = rows.map((row) => {
        ...
        const transferRenewalAmount = toNumericPayrollValue(getValue(PAYROLL_EXCEL_HEADERS.transfer_renewal_amount))
        ...
        const legacyDeductionsAmount = toNumericPayrollValue(getValue(PAYROLL_EXCEL_HEADERS.deductions_amount))
        const legacyInstallmentAmount = toNumericPayrollValue(getValue(PAYROLL_EXCEL_HEADERS.installment_deducted_amount))

        return {
          residence_number: normalizeResidenceNumber(getValue(PAYROLL_EXCEL_HEADERS.residence_number)),
          ...
          installment_deducted_amount: transferRenewalAmount + advanceAmount || legacyInstallmentAmount,
      ...
      for (const [index, row] of normalizedRows.entries()) {
        ...
        if (!employee) {
          importErrors.push(`الصف ${index + 2}: لا يوجد موظف ضمن نطاق المسير برقم إقامة ${row.residence_number}`)
          continue
        }
        ...
        const { dailyRate: employeeDailyRate, grossAmount: importedGrossAmount, netAmount: importedNetAmount } = calculatePayrollTotals(
      ...
      if (importErrors.length > 0) {
        ...
        toast.warning(`تمت معاينة ${previewRows.length} صف صالح، وتعذر تجهيز ${importErrors.length} صف`)
  ...
  const getPayrollRunDisplayName = (scopeType: PayrollScopeType, scopeId: string, payrollMonth: string) => {
  ...
  const getExistingRunWarningMessage = (run: { status: string; scope_type: PayrollScopeType; scope_id: string; payroll_month: string }) => {
  ...
  const handleTogglePayrollRunForm = () => {
    if (!showPayrollRunForm) {
      ...
      setPayrollForm((current) => ({
        ...
        scope_id: defaultScopeOptions.some((item) => item.id === current.scope_id) && current.scope_type === defaultScopeType
          ? current.scope_id
  ...
  const handleOpenPayrollEntryForm = () => {
    ...
    if (!selectedPayrollRunEditable) {
      toast.error(selectedPayrollRun.status === 'cancelled'
    ...
    if (scopedPayrollEmployees.length === 0) {
      toast.warning('لا يوجد موظفون داخل نطاق هذا المسير حاليًا. أضف موظفًا للنطاق أولاً ثم حاول مرة أخرى.')
      return
    }
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="app-panel mb-5 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600">هذه الصفحة مخصصة لمسيرات الرواتب، إدخال الرواتب، الاستيراد، القسائم، والاستقطاعات فقط.</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-5 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="text-sm text-slate-500 mb-1">إجمالي الالتزامات</div>
                <div className="text-2xl font-bold text-slate-900">{obligationStats.total.toLocaleString('en-US')}</div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="text-sm text-slate-500 mb-1">ما تم سداده فعلياً</div>
                <div className="text-2xl font-bold text-green-600">{obligationStats.paid.toLocaleString('en-US')}</div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="text-sm text-slate-500 mb-1">المتبقي الفعلي</div>
                <div className="text-2xl font-bold text-red-600">{obligationStats.remaining.toLocaleString('en-US')}</div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="text-sm text-slate-500 mb-1">المسدد فعلياً في الشهر</div>
                <div className="text-2xl font-bold text-blue-700">{filteredObligationInsightRows.reduce((sum, row) => sum + Number(row.amount_paid || 0), 0).toLocaleString('en-US')}</div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-600">اكتب أي رقم أو اسم أو مشروع وسيتم الفلترة مباشرة.</p>
                <p className="mt-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  الأرقام داخل الصف تمثل قسط أو استقطاع المسير لهذا الشهر، أما المتبقي بالأعلى فهو الرصيد الفعلي من خطة الالتزام ولا ينخفض إلا بعد اعتماد المسير نهائياً.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium text-slate-700">المشروع</label>
                  <select
                    ...
                    <tbody>
                      {filteredPayrollSearchRows.map((row) => (
                        <tr key={row.id} className="border-t hover:bg-slate-50">
                          <td className="px-4 py-3 font-medium text-slate-900">{row.employee_name_snapshot}</td>
                          ...
                          <td className="px-4 py-3">{row.deduction_breakdown.transfer_renewal.toLocaleString('en-US')}</td>
                          <td className="px-4 py-3">{row.deduction_breakdown.penalty.toLocaleString('en-US')}</td>
                          <td className="px-4 py-3">{row.deduction_breakdown.advance.toLocaleString('en-US')}</td>
                          <td className="px-4 py-3">{row.deduction_breakdown.other.toLocaleString('en-US')}</td>
                          <td className="px-4 py-3 font-semibold text-red-600">{row.total_deductions.toLocaleString('en-US')}</td>
                          <td className="px-4 py-3 font-semibold text-amber-700">{row.obligation_remaining.toLocaleString('en-US')}</td>
          ...
          <div className="space-y-6">
              <div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-5 space-y-4">
                <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">إحصائيات مسيرات الرواتب</h2>
                    <p className="text-sm text-slate-600">اختر شهرًا أو مسيرًا محددًا وستتغير الكروت مباشرة.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 w-full lg:w-auto lg:min-w-[760px]">
                    <div>
                      <label className="mb-2 block text-sm font-medium text-slate-700">فلتر الشهر</label>
                      <input
                        type="month"
                    ...
                    <div>
                      <label className="mb-2 block text-sm font-medium text-slate-700">فلتر المسير</label>
                      <select
                      ...
                      >
                        ...
                        {payrollRunList.map((run) => (
                          <option key={run.id} value={run.id}>
                            {getPayrollRunDisplayName(run.scope_type, run.scope_id, run.payroll_month)}
                          </option>
                        ))}
                      </select>
                    </div>
                    <div className="flex items-end">
                      <button
                        ...
                        className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 transition"
                      >
                        إعادة ضبط الفلتر
                      </button>
                    </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-gray-900">{filteredPayrollRunList.length}</p>
                      </div>
                      <div className="app-icon-chip">
                        <Wallet className="w-6 h-6" />
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-sky-700">{payrollRunCardsStats.employees}</p>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">{payrollRunCardsStats.gross.toLocaleString('en-US')}</p>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-red-600">{payrollRunCardsStats.totalObligations.toLocaleString('en-US')}</p>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-amber-600">{payrollRunCardsStats.transferRenewal.toLocaleString('en-US')}</p>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-rose-600">{payrollRunCardsStats.penalty.toLocaleString('en-US')}</p>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-blue-700">{payrollRunCardsStats.advance.toLocaleString('en-US')}</p>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-violet-700">{payrollRunCardsStats.other.toLocaleString('en-US')}</p>
                      </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 md:p-5 space-y-4">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">أنشئ مسيرًا جديدًا لمؤسسة أو مشروع، ثم راجع كشف الموظفين المرتبط به</p>
                  </div>
                  {isAdmin && (
                    <button
                ...
                {showPayrollRunForm && isAdmin && (
                  <div className="grid grid-cols-1 gap-4 rounded-lg border border-primary/30 bg-primary/10 p-4 md:grid-cols-2 lg:grid-cols-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">شهر الرواتب</label>
                      <input
                        type="month"
                        value={payrollForm.payroll_month}
                        onChange={(e) => setPayrollForm((current) => ({ ...current, payroll_month: e.target.value }))}
                        className="w-full px-3 py-2 border rounded-md"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">نوع المسير</label>
                      <select
                        value={payrollForm.scope_type}
                        onChange={(e) => setPayrollForm((current) => ({ ...current, scope_type: e.target.value as PayrollScopeType, scope_id: '' }))}
                        className="w-full px-3 py-2 border rounded-md"
                      >
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{payrollForm.scope_type === 'project' ? 'اختر المشروع' : 'اختر المؤسسة'}</label>
                      <select
                        value={payrollForm.scope_id}
                        onChange={(e) => setPayrollForm((current) => ({ ...current, scope_id: e.target.value }))}
                        className="w-full px-3 py-2 border rounded-md"
                      >
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">طريقة الإدخال</label>
                      <select
                        value={payrollForm.input_mode}
                        onChange={(e) => setPayrollForm((current) => ({ ...current, input_mode: e.target.value as PayrollInputMode }))}
                        className="w-full px-3 py-2 border rounded-md"
                      >
                    ...
                    <div className="md:col-span-2 lg:col-span-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
                      ...
                      {payrollForm.scope_id && (
                        <div className="mt-2 font-bold">
                          سيتم إنشاء: {getPayrollRunDisplayName(payrollForm.scope_type, payrollForm.scope_id, payrollForm.payroll_month)}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="md:col-span-2 lg:col-span-4 flex justify-end gap-3">
                      ...
                      <button
                        ...
                        disabled={createPayrollRun.isPending}
                      >
                        {createPayrollRun.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                        إنشاء المسير
                      </button>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
                  <div className="xl:col-span-2 border border-gray-200 rounded-lg overflow-hidden">
                    <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div className="font-medium text-gray-800">قائمة المسيرات ({payrollRunStatsRows.length} مدخل رواتب)</div>
                        {canExport('payroll') && filteredPayrollRunList.length > 0 && (
                          <div className="flex flex-wrap items-center gap-2">
                            <label className="inline-flex items-center gap-2 text-xs text-gray-700 bg-white border border-gray-200 rounded-md px-3 py-2">
                              <input
                                type="checkbox"
                            ...
                            <button
                              ...
                              disabled={selectedPayrollExportRunIds.length === 0 || exportingSelectedPayrollRuns}
                              className={`${successCompactButtonClass} bg-emerald-600 hover:bg-emerald-700`}
                            >
                              {exportingSelectedPayrollRuns ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                    ...
                    <div className="max-h-[520px] overflow-y-auto">
                      {payrollRunsLoading ? (
                        <div className="p-6 text-center text-gray-500">جاري تحميل مسيرات الرواتب...</div>
                      ) : filteredPayrollRunList.length === 0 ? (
                        <div className="p-6 text-center text-gray-500">لا توجد مسيرات مطابقة للفلاتر الحالية.</div>
                      ) : (
                        filteredPayrollRunList.map((run) => (
                          <div key={run.id} className={`flex items-start gap-3 border-b border-gray-100 p-3 ${selectedPayrollRunId === run.id ? 'bg-blue-50' : ''}`}>
                            {canExport('payroll') && (
                              <label className="mt-1 inline-flex items-center">
                                <input
                                  ...
                                  onChange={(event) => handleTogglePayrollRunExportSelection(run.id, event.target.checked)}
                                  className="rounded border-gray-300"
                                />
                              </label>
                            )}
                            <button
                              ...
                              className={`flex-1 text-right p-3 rounded-lg transition ${selectedPayrollRunId === run.id ? 'bg-white border border-blue-200 border-r-4 border-r-blue-600' : 'hover:bg-gray-50'}`}
                            >
                              <div className="flex items-start justify-between gap-2 mb-2">
                                <span className="font-medium text-gray-900">{formatPayrollMonthLabel(run.payroll_month)}</span>
                                <span className={`px-2 py-1 rounded-full text-xs ${run.status === 'finalized' ? 'bg-green-100 text-green-700' : run.status === 'draft' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-700'}`}>
                                  {getPayrollStatusText(run.status)}
                                </span>
                              </div>
                              {selectedPayrollRunId === run.id && (
                                <div className="text-xs font-medium text-blue-700 mb-2">المسير المحدد الآن</div>
                              )}
                              <div className="text-sm text-gray-600">
                                {getPayrollRunDisplayName(run.scope_type, run.scope_id, run.payroll_month)}
                              </div>
                              <div className="text-xs text-gray-500 mt-1">طريقة الإدخال: {getPayrollInputModeText(run.input_mode)}</div>
                              <div className="text-xs text-gray-500 mt-1">{run.entry_count} موظف • صافي {run.total_net_amount.toLocaleString('en-US')}</div>
                            </button>
                          </div>
                        ))
                      )}
                  ...
                  <div ref={payrollDetailsPanelRef} className="xl:col-span-3 border border-gray-200 rounded-lg overflow-hidden bg-white">
                    <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-3">
                      <div>
                        <div className="font-medium text-gray-800">تفاصيل المسير</div>
                        {selectedPayrollRun && (
                          <div className="text-sm text-gray-500 mt-1">
                            {getPayrollRunDisplayName(selectedPayrollRun.scope_type, selectedPayrollRun.scope_id, selectedPayrollRun.payroll_month)} • {selectedPayrollRun.entry_count} موظف
                          </div>
                        )}
                        {selectedPayrollRun && (
                          <div className="text-xs text-gray-500 mt-1">
                            قسائم الرواتب المولدة: {payrollSlips.length} • طريقة الإدخال: {getPayrollInputModeText(selectedPayrollRun.input_mode)}
                          </div>
                        )}
                        {selectedPayrollRun && (
                          <div className="flex flex-wrap items-center gap-2 mt-2">
                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${selectedPayrollRun.status === 'finalized' ? 'bg-green-100 text-green-700' : selectedPayrollRun.status === 'draft' ? 'bg-orange-100 text-orange-700' : selectedPayrollRun.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
                            ...
                            {selectedPayrollRunEditable && scopedPayrollEmployees.length === 0 && !scopedEmployeesLoading && (
                      ...
                      {selectedPayrollRun && isAdmin && (
                        <div className="flex flex-wrap items-center justify-end gap-2 max-w-xl">
                          ...
                          <button
                            ...
                            className={`${primaryCompactButtonClass} disabled:bg-slate-200 disabled:text-slate-500 disabled:border disabled:border-slate-200`}
                            disabled={!selectedPayrollRunEditable || scopedEmployeesLoading || scopedPayrollEmployees.length === 0}
                            title={selectedPayrollRun?.status === 'cancelled' ? 'هذا المسير ملغي ويجب إعادة فتحه أولًا' : scopedPayrollEmployees.length === 0 ? 'لا يوجد موظفون داخل نطاق المسير الحالي' : undefined}
                          >
                            <Plus className="w-4 h-4" />
                          ...
                          {selectedPayrollRunEditable && (
                            <button
                              ...
                              disabled={importingPayrollExcel || confirmingPayrollExcelImport || scopedPayrollEmployees.length === 0}
                            >
                              {importingPayrollExcel ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileUp className="w-4 h-4" />}
                              استيراد Excel
                            </button>
                          )}
                          {selectedPayrollRunEditable && (
                            ...
                            >
                              {updatePayrollRunStatus.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <ReceiptText className="w-4 h-4" />}
                              اعتماد المسير
                            </button>
                          )}
                          {selectedPayrollRun.status === 'finalized' && (
                            <button
                              ...
                              disabled={updatePayrollRunStatus.isPending}
                            >
                              {updatePayrollRunStatus.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                              إعادة إلى مسودة
                            </button>
                          )}
                          ...
                          {selectedPayrollRun.status === 'cancelled' && canDelete('payroll') && (
                            ...
                            >
                              {deletePayrollRun.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                              حذف المسير
                            </button>
                          )}
                          {selectedPayrollRun.status !== 'cancelled' && (
                            <button
                              ...
                              disabled={updatePayrollRunStatus.isPending}
                            >
                              {updatePayrollRunStatus.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <AlertTriangle className="w-4 h-4" />}
                              إلغاء المسير
                            </button>
                          )}
                        </div>
                    ...
                    {selectedPayrollRun && showPayrollEntryForm && isAdmin && (
                      <div ref={payrollEntryFormRef} className="p-4 border-b border-gray-200 bg-blue-50 space-y-4">
                        <div className="rounded-lg border border-blue-200 bg-white px-4 py-3 text-sm text-gray-700">
                          أدخل راتب الموظف يدويًا داخل المسير الحالي. إذا كان لهذا الموظف مدخل سابق في نفس المسير، فالحفظ سيقوم بالتحديث بدل إنشاء سجل مكرر.
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                setPayrollEntryForm((current) => ({
                                  ...
                                  other_amount: 0,
                                  installment_deducted_amount: employee?.suggested_installment_amount ?? 0,
                                }))
                              }}
                              className="w-full px-3 py-2 border rounded-md"
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">أيام الحضور</label>
                            <input
                              type="number"
                              min="0"
                              step="0.5"
                              value={payrollEntryForm.attendance_days}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, attendance_days: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">الإجازات المدفوعة</label>
                            <input
                              type="number"
                              min="0"
                              step="0.5"
                              value={payrollEntryForm.paid_leave_days}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, paid_leave_days: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, basic_salary_snapshot: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">الإضافي</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              value={payrollEntryForm.overtime_amount}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, overtime_amount: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">قسط رسوم نقل وتجديد</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              value={payrollEntryForm.transfer_renewal_amount}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, transfer_renewal_amount: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">قسط جزاءات وغرامات</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              value={payrollEntryForm.penalty_amount}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, penalty_amount: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">قسط سلفة</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              value={payrollEntryForm.advance_amount}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, advance_amount: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">قسط أخرى</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              value={payrollEntryForm.other_amount}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, other_amount: Number(e.target.value) || 0 }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">الأجر اليومي</label>
                            <div className="w-full px-3 py-2 border rounded-md bg-white text-gray-700">{dailyRate.toLocaleString('en-US')}</div>
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">ملاحظات الإضافي</label>
                            <input
                              type="text"
                              value={payrollEntryForm.overtime_notes}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, overtime_notes: e.target.value }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">ملاحظات الاستقطاعات</label>
                            <input
                              type="text"
                              value={payrollEntryForm.deductions_notes}
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, deductions_notes: e.target.value }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              onChange={(e) => setPayrollEntryForm((current) => ({ ...current, notes: e.target.value }))}
                              className="w-full px-3 py-2 border rounded-md"
                            />
                          </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div className="bg-white border border-gray-200 rounded-lg p-3">
                            ...
                            <div className="text-lg font-bold text-gray-900">{grossAmount.toLocaleString('en-US')}</div>
                          </div>
                          <div className="bg-white border border-gray-200 rounded-lg p-3">
                            <div className="text-sm text-gray-500 mb-1">إجمالي الاستقطاعات</div>
                            <div className="text-lg font-bold text-red-600">{groupedDeductionsTotal.toLocaleString('en-US')}</div>
                          </div>
                          <div className="bg-white border border-gray-200 rounded-lg p-3">
                            <div className="text-sm text-gray-500 mb-1">الصافي</div>
                            <div className={`text-lg font-bold ${netAmount < 0 ? 'text-red-600' : 'text-blue-700'}`}>{netAmount.toLocaleString('en-US')}</div>
                          </div>
                          <div className="bg-white border border-gray-200 rounded-lg p-3">
                            <div className="text-sm text-gray-500 mb-1">اقتراح الأقساط</div>
                            <div className="text-lg font-bold text-orange-600">{(selectedPayrollEmployee?.suggested_installment_amount ?? 0).toLocaleString('en-US')}</div>
                          </div>
                        </div>
                        <div className="flex justify-end gap-3">
                          ...
                          <button
                            ...
                            disabled={upsertPayrollEntry.isPending}
                          >
                            {upsertPayrollEntry.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <ReceiptText className="w-4 h-4" />}
                            حفظ راتب الموظف
                          </button>
                        </div>
                      </div>
                    ...
                    {!selectedPayrollRun ? (
                      ...
                      <div className="p-8 bg-slate-50 border-t border-gray-100">
                        <div className="max-w-lg mx-auto text-center space-y-4">
                          <div className={`mx-auto w-14 h-14 rounded-2xl flex items-center justify-center ${selectedPayrollRun.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
                            {selectedPayrollRun.status === 'cancelled' ? <AlertTriangle className="w-7 h-7" /> : <Wallet className="w-7 h-7" />}
                          </div>
                          <div className="space-y-2">
                            <h3 className="text-lg font-semibold text-slate-900">
                              {selectedPayrollRun.status === 'cancelled' ? 'هذا المسير ملغي حاليًا' : 'المسير المحدد جاهز لإدخال الرواتب'}
                            </h3>
                          ...
                          {selectedPayrollRun.status !== 'cancelled' && (
                            <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-right">
                              <div className="text-sm font-semibold text-slate-900 mb-2">للبدء السريع:</div>
                          ...
                          {selectedPayrollRunEditable && isAdmin && scopedPayrollEmployees.length === 0 && !scopedEmployeesLoading && (
                      ...
                      <div className="border-t border-gray-200 bg-slate-50 px-4 py-4">
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                          <div className="lg:max-w-sm">
                            <h3 className="font-semibold text-slate-900 mb-1">استيراد الرواتب من Excel</h3>
                            <p className="text-sm text-slate-600">
                              ابدأ بالقالب الجاهز، ثم ارفع الملف وراجع الصفوف قبل الاعتماد النهائي داخل نفس المسير.
                            </p>
                          </div>
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                          <div>
                            ...
                            <p className="text-sm text-blue-700 mt-1">
                              الملف: {payrollImportFileName || 'Excel'} • الصفوف الجاهزة للاعتماد: {payrollImportPreviewRows.length}
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              disabled={confirmingPayrollExcelImport}
                            >
                              {confirmingPayrollExcelImport ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                              اعتماد الاستيراد
                            </button>
                          </div>
                        </div>
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {payrollImportPreviewRows.map((row) => (
                                <tr key={`${row.employee_id}-${row.row_number}`} className="border-t border-blue-100">
                      ...
                      <div className="border-t border-gray-200 bg-red-50 px-4 py-4">
                        <div className="flex items-center justify-between gap-3 mb-3">
                          <div>
                            ...
                            <p className="text-sm text-red-700 mt-1">تم استيراد بعض الصفوف، لكن الصفوف التالية تحتاج تصحيحًا قبل إعادة الرفع.</p>
                          </div>
                          <button
                            type="button"
                        ...
                        <div className="max-h-48 overflow-y-auto rounded-lg border border-red-200 bg-white">
                          <ul className="divide-y divide-red-100 text-sm text-red-800">
                            {payrollImportErrors.map((error, index) => (
                              <li key={`${error}-${index}`} className="px-4 py-3">{error}</li>
                            ))}
                          </ul>
                        </div>
                      </div>
        ...
        {payrollRunDeleteConfirmOpen && selectedPayrollRun && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
            <div role="dialog" aria-modal="true" className="w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4">
                <div>
                  <h2 className="text-lg font-bold text-slate-900">تأكيد حذف المسير</h2>
                  <p className="mt-1 text-sm text-slate-600">{getPayrollRunDisplayName(selectedPayrollRun.scope_type, selectedPayrollRun.scope_id, selectedPayrollRun.payroll_month)}</p>
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-3 px-5 py-4">
                ...
                <p className="text-sm text-slate-600">إذا كنت متأكدًا، اضغط على تأكيد الحذف.</p>
              </div>
              <div className="flex items-center justify-end gap-2 border-t border-slate-200 px-5 py-4">
                ...
                <button
                  ...
                  disabled={deletePayrollRun.isPending}
                >
                  {deletePayrollRun.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                  تأكيد الحذف
                </button>
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm overflow-y-auto">
            <div className="app-modal-surface w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              <div className="app-modal-header flex items-center justify-between gap-4 px-6 py-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {selectedPayrollSlip.slip_number} • {selectedSlipEntry.employee_name_snapshot || 'موظف'}
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="p-6 space-y-6">
                ...
                <div ref={payrollSlipPreviewRef} className="space-y-6 bg-white">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                      <div className="text-sm text-gray-500 mb-1">الموظف</div>
                      <div className="font-semibold text-gray-900">{selectedSlipEntry.employee_name_snapshot || '-'}</div>
                    </div>
                    <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                      <div className="text-sm text-gray-500 mb-1">رقم الإقامة</div>
                      <div className="font-semibold text-gray-900">{selectedSlipEntry.residence_number_snapshot || '-'}</div>
                    </div>
                    <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                      <div className="text-sm text-gray-500 mb-1">تاريخ التوليد</div>
                      <div className="font-semibold text-gray-900">{selectedPayrollSlip.generated_at ? new Date(selectedPayrollSlip.generated_at).toLocaleString('en-GB') : '-'}</div>
                    </div>
                    <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                      <div className="text-sm text-gray-500 mb-1">نسخة القالب</div>
                      <div className="font-semibold text-gray-900">{selectedPayrollSlip.template_version}</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                      ...
                      <div className="text-xl font-bold text-blue-900">{Number(selectedSlipTotals?.grossAmount || selectedSlipEntry.gross_amount || 0).toLocaleString('en-US')}</div>
                    </div>
                    <div className="rounded-xl border border-red-200 bg-red-50 p-4">
                      <div className="text-sm text-red-700 mb-1">الخصومات</div>
                      <div className="text-xl font-bold text-red-900">{Number(selectedSlipEntry.deductions_amount || 0).toLocaleString('en-US')}</div>
                    </div>
                    <div className="rounded-xl border border-orange-200 bg-orange-50 p-4">
                      <div className="text-sm text-orange-700 mb-1">خصم الأقساط</div>
                      <div className="text-xl font-bold text-orange-900">{Number(selectedSlipEntry.installment_deducted_amount || 0).toLocaleString('en-US')}</div>
                    </div>
                    <div className="rounded-xl border border-green-200 bg-green-50 p-4">
                      <div className="text-sm text-green-700 mb-1">صافي الراتب</div>
                      <div className="text-xl font-bold text-green-900">{Number(selectedSlipTotals?.netAmount || selectedSlipEntry.net_amount || 0).toLocaleString('en-US')}</div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    {selectedSlipComponents.length === 0 ? (
                      <div className="p-6 text-sm text-gray-500 text-center">لا توجد مكونات تفصيلية محفوظة لهذه القسيمة.</div>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead className="bg-gray-50">
                          ...
                          <tbody>
                            {selectedSlipComponents.map((component, index) => (
                              <tr key={`${component.component_code || 'component'}-${index}`} className="border-t">
                                <td className="px-4 py-3">{component.component_type || '-'}</td>
                                <td className="px-4 py-3 font-medium text-gray-900">{component.component_code || '-'}</td>
                                <td className="px-4 py-3">{Number(component.amount || 0).toLocaleString('en-US')}</td>
                                <td className="px-4 py-3 text-gray-600">{component.notes || '-'}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
